    ...
    function loadFile(file, callback) {
      ...
      util.timer(2000, function() {
          if (!s.loaded) {
            ...
            // TODO(errge): alert was too strong, once we have some
            // internal popup/hint UI thingie, we can use that.  If
            // the load finishes, we can get rid of the warning then.

            util.c.debug("Script " + file + " failed to load after 2 seconds! " +
...
jQuery.parseQuery = function(qs,options) {
  ...
  var unesc = function(v) { return unescape(v).replace(/\+/g,' '); };
  var params = {};
  var key, val;
