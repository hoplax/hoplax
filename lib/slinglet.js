...
// sets completion div's height to fill the remaining space in the window under the header
Completer.setCompletionHeight = function() {
  var windowHeight = $(window).height();
  var completionTop = $("#completion").offset().top;
  var headerTop = $("#header").offset().top;
  $("#completion").css("height", (windowHeight - completionTop - headerTop) + "px");
};
...
Completer.prototype.complete = function(runaction, i) {
  ...
  if (i > 0) { // user wants to use the result of autocompletion
    ...
    if (this.inputhandler.val() == "") {
      ...
      if (runaction) this.action(plainInput); // callback with the input text
    }
  }
};
...
Slinglet.prototype.activate = function() {
  ...
  // have to reset the height of the completion div.
