...
Cursor.prototype.reset = function(first, last, n) {
  ...
  this.n_ = n;
  // position the second choice by default
  if (n > 1) this.next();
};
...
/**
 * Computes the result of the completion: return the first column.
 * Virtual.
 *
 * @param {object} values map from dataset ids to currently selected values.
 *
 * @return {string} the result of the completion.
 */
Completer.prototype.useRow = function(values) {
  return values[this.dataset_ids[0]];
};

/**
 * Computes the completion result if the user selected her own input.
 * Virtual.
 *
 * @param {string} s user's string.
 * @return {string} the (maybe mangled) user string.
 */
Completer.prototype.usePlainInput = function(s) {
  return s;
};
...
Completer.prototype.complete = function(i) {
  ...
  if (i > 0) { // user wants to use the result of autocompletion
    var values = this.cached_table[i - 1][1];
    this.inputhandler.val(this.useRow(values));
    this.action(values); // callback to the slinglet with the selected datarow
  } else {
    if (this.inputhandler.val()) {
      var plainInput = this.usePlainInput(this.inputhandler.val());
      this.inputhandler.val(plainInput);
      this.action(plainInput); // callback with the input text
    } else {
      this.emptyAction();
    }
  }
...
/**
 * Value to show for empty input in the completion table.  Virtual.
 */
Completer.prototype.emptyText = 'No value, thanks!';

/**
 * Action to do on empty input.  Virtual.
 */
...
Completer.prototype.fillTableEnd = function(str) {
  ...
  ui.append(user_row);
  user_row.click(function() { self.complete(0); });
  ...
  this.cached_table.forEach(function(r, i) {
    ...
    row.click(function() { self.complete(i + 1); });
    ui.append(row);
  });
