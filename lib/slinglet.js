...
function InputBox(id, label, width) {
  ...
  this.inputhandler.focus(function (event) {
...
function Completer(id, label, width, dataset, dataset_labels, dataset_ids) {
  ...
  this.inputhandler.focus(function () {
    ...
    self.fillTableRequestedSinceFocus = false;
  });
...
Completer.prototype.requestFillTable = function(immediately) {
  this.fillTableRequestedSinceFocus = true;
...
Completer.prototype.inputHandlerKeydownBindme = function (event) {
  ...
  if (event.which == 9 && event.shiftKey == false) { // tab without shift
    if (this.fillTableRequestedSinceFocus) {
      this.joinFillTable();
      this.complete(false);
    }
  }
