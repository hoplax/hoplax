...
/**
 * Convert a user supplied string to an URL.  Needed for the generation
 * of the 'Jump to:' row.
 */
hoplax.urlize = function(s) {
  if (s.match(/^\//)) // absolute path name
    return "file://" + s;
  else if (s.match(/^[a-z]+:/)) // protocol given
    return s;
  else // urlize by prepending http://
    return "http://" + s;
};

/**
 * With empty input (at start) offer the help page, with some input
 * offer direct jump and Google search.
 */
hoplax.bookmark_completer.plainHandlers = function(s) {
  if (s == '')
    return [
      {
        s: 'First time user?  Navigate here or click here for help!',
        f: function() {
    ...
    return [
      ...
      },
      {
        s: 'Google search: ' + s,
        f: function() {
          window.location = 'http://www.google.com/search?q=' + encodeURIComponent(s);
        }
      }
    ];
};
...
/**
 * The selected row is stored in completed_row, activate that slinglet.
 */
hoplax.bookmark_completer.action = function() {
  var o = this.completed_row;
